(() => {
  ...
  const applyVersion = () => {
    ...
    if (badge && badge.textContent !== `PREVIEW v${VERSION}`) {
      badge.textContent = `PREVIEW v${VERSION}`;
    }
    ...
    if (warn && !warn.innerHTML.includes(`v${VERSION}`)) {
      warn.innerHTML = `<strong>v${VERSION}:</strong> נוספה בקרת אורך חיה. טקסט ארוך לא נחתך אוטומטית - המחולל מזהיר כשהניסוח מתחיל לפגוע בהיררכיה של השקופית.`;
    }
  };
